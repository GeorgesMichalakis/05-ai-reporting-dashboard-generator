...
async function enrichWithOpenAI(fallback: AutomationRun): Promise<AutomationRun> {
  ...
  const prompt = [
    `You are an AI automation consultant demoing AI Reporting Dashboard Generator.`,
...
app.listen(port, () => {
  console.log(`AI Reporting Dashboard Generator listening on http://localhost:${port}`);
});
